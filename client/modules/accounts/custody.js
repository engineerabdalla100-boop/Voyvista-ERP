  ...
  function renderVouchersTable() {
    ...
    tbody.innerHTML = rows.map((voucher) => `
      <tr class="${voucher.isReversal ? "row-reversal" : ""}">
        ...
        <td>${(voucher.status === "posting" || voucher.reversalState === "in_progress") ? `<span class="badge badge--pending" title="عملية سابقة لم تكتمل — يحتاج مراجعة يدوية">⚠️ يحتاج مراجعة</span>` : WF.actionsHtml(voucher, { allowReverse: true })}</td>
      </tr>
    `).join("");
  ...
  if (window.VVChangeRequests) {
    window.VVChangeRequests.registerApplyHandler("vouchers", (request) => {
      if (window.VVPermissions && window.VVAuth) {
        ...
        const requiredPermission = window.VVPermissions.permissionForAction({ module: "vouchers", changeType: request.changeType, payload: request.payload });
