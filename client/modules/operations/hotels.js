document.addEventListener("DOMContentLoaded", () => {
  // State Storage (separate keys from Flights — independent ledgers per department)
  let salesData = JSON.parse(localStorage.getItem("vv_hotel_sales_data")) || [];
  let requestsData = JSON.parse(localStorage.getItem("vv_hotel_requests_data")) || [];
  let editingSaleId = null;
  let editingRequestId = null;
  let nextCodeId = parseInt(localStorage.getItem("vv_hotel_next_code_id")) || 1001;
  ...
  const generateNewFamilyCode = () => `FAM-H${nextCodeId}`;
  ...
  const setupInvoiceLogic = () => {
    ...
    document.getElementById("btn-print-voucher")?.addEventListener("click", () => {
      ...
      doc.setFontSize(16);
      doc.text("Voyvista — Hotels", marginX, y);
      doc.setFontSize(13);
      ...
      doc.save(`Voyvista-Hotel-Invoice-${data.ref || data.date}.pdf`);
    });
  ...
  // Net / Net Rate stay manual (per the hotels team's own sheet), but
  // Profit is now auto-calculated: Profit = Selling Rate - Net Rate.
  const calcProfit = (netRateId, sellingId, profitId) => {
    const netRate = parseFloat(document.getElementById(netRateId).value) || 0;
    const selling = parseFloat(document.getElementById(sellingId).value) || 0;
    document.getElementById(profitId).value = (selling - netRate).toFixed(2);
  };

  const initForms = () => {
    document.getElementById("sales-checkin").value = getTodayISO();
    document.getElementById("req-checkin").value = getTodayISO();
    ...
    ["sales-net-rate", "sales-selling-rate"].forEach(id => {
      document.getElementById(id)?.addEventListener("input", () => {
        calcProfit("sales-net-rate", "sales-selling-rate", "sales-profit");
      });
    });
    ["req-net-rate", "req-selling-rate"].forEach(id => {
      document.getElementById(id)?.addEventListener("input", () => {
        calcProfit("req-net-rate", "req-selling-rate", "req-profit");
      });
    });
    ...
    // Real link to the central customer/company registry (data.js's
    // vv_acc_party_meta) — the same store the B2B/B2C DNA system reads
    // and writes. Picking a code here is picking a REAL party in that
    // system, not a locally-invented code isolated to this one file.
    ...
    toggleCheckbox?.addEventListener("change", (e) => {
      if (e.target.checked) {
        ...
        // One option per unique code, showing every name that shares it
        // (a company code lists all its employees; a family code lists
        // everyone in the family) so picking the right one is obvious.
        const namesByCode = {};
        ...
        codeSelect.innerHTML = '<option value="">-- Select Code --</option>' +
          codes.map((code) => `<option value="${code}">${code} — ${namesByCode[code].join(" / ")}</option>`).join("");
        codeSelect.dataset.namesByCode = JSON.stringify(namesByCode);
        codeInput.style.display = "none";
        codeSelect.style.display = "block";

        if (codes.length === 0) {
          codeSelect.innerHTML = '<option value="">-- لا توجد أكواد مسجّلة في قسم البيانات بعد --</option>';
        }
      } else {
    ...
    // Picking a code auto-fills the client name field if it's empty —
    // never overwrites something the user already typed, just saves a
    // step when starting fresh from a known customer/company.
    codeSelect?.addEventListener("change", () => {
      ...
      const clientNameField = document.getElementById("sales-client-name");
      if (clientNameField && !clientNameField.value.trim() && names.length === 1) {
        clientNameField.value = names[0];
      }
    });
  };
  ...
  // =========================================================================
  // Payment switch — Cash / InstaPay / Partial, directly on each Sales
  // Report row. Writes to the EXACT SAME fields (collectionStatus,
  // paidAmount, remainingAmount) that Data's Accounting/Arrears sections
  // already read from vv_hotel_sales_data — there is no separate sync
  // step needed, because it's literally the same record. Marking
  // something paid here shows up correctly in Data's arrears list and
  // Sales Report Balance immediately, and vice versa.
  //
  // Lives in the registration form itself, as basic registration data —
  // NOT as interactive controls in the Sales Report table. The table
  // only ever shows a read-only status badge summarizing what was
  // chosen. HONEST LIMITATION (deliberate): documentation field only —
  ...
  const buildSalesRows = (tbody, filtered) => {
    ...
    filtered.forEach((item) => {
      const itemDate = parseLocalDate(item.checkIn);
      ...
      if (monthYear !== currentMonthYear) {
        ...
        separatorRow.innerHTML = `<td colspan="18">--- ${currentMonthYear.toUpperCase()} ---</td>`;
      ...
      row.innerHTML = `
        ...
        <td>${item.location || '-'}</td>
        <td class="num">${parseFloat(item.net || 0).toFixed(2)}</td>
  ...
  const renderSalesTable = (filterQuery = "") => {
    ...
    if (tbody) {
      ...
      if (isDefaultLatestView) {
        filtered.sort((a, b) => parseLocalDate(b.checkIn) - parseLocalDate(a.checkIn));
        filtered = filtered.slice(0, 10);
      }
      filtered.sort((a, b) => parseLocalDate(a.checkIn) - parseLocalDate(b.checkIn));
      buildSalesRows(tbody, filtered);
    }

    const bookingTbody = document.getElementById("booking-sales-report-table-body");
    if (bookingTbody) {
      const full = [...salesData].sort((a, b) => parseLocalDate(a.checkIn) - parseLocalDate(b.checkIn));
  ...
  document.getElementById("sales-form")?.addEventListener("submit", (e) => {
    ...
    const newSale = {
      id: saleId,
      checkIn: document.getElementById("sales-checkin").value,
      checkOut: document.getElementById("sales-checkout").value,
      invoice: document.getElementById("sales-invoice").value,
      clientName: document.getElementById("sales-client-name").value,
      hotelName: document.getElementById("sales-hotel-name").value,
      location: document.getElementById("sales-location").value,
      net: document.getElementById("sales-net").value,
      ...
      discountNotice: document.getElementById("sales-discount-notice").value,
    ...
    if (!wasEditing && !isExisting) {
      nextCodeId++;
      localStorage.setItem("vv_hotel_next_code_id", nextCodeId);
    }
    localStorage.setItem("vv_hotel_sales_data", JSON.stringify(salesData));
  ...
  document.getElementById("requests-form")?.addEventListener("submit", (e) => {
    ...
    const newReq = {
      id: reqId,
      checkIn: document.getElementById("req-checkin").value,
      checkOut: document.getElementById("req-checkout").value,
      invoice: document.getElementById("req-invoice").value,
      clientName: document.getElementById("req-client-name").value,
      hotelName: document.getElementById("req-hotel-name").value,
      location: document.getElementById("req-location").value,
      net: document.getElementById("req-net").value,
      ...
      discountNotice: document.getElementById("req-discount-notice").value,
    ...
    localStorage.setItem("vv_hotel_requests_data", JSON.stringify(requestsData));
  ...
  window.editSaleRecord = (id) => {
    ...
    document.getElementById("sales-checkin").value = sale.checkIn;
    document.getElementById("sales-checkout").value = sale.checkOut;
    document.getElementById("sales-invoice").value = sale.invoice;
    document.getElementById("sales-client-name").value = sale.clientName;
    document.getElementById("sales-hotel-name").value = sale.hotelName;
    document.getElementById("sales-location").value = sale.location;
    document.getElementById("sales-net").value = sale.net;
    ...
    document.getElementById("sales-discount-notice").value = sale.discountNotice;
  ...
  window.editRequestRecord = (id) => {
    ...
    document.getElementById("req-checkin").value = req.checkIn;
    document.getElementById("req-checkout").value = req.checkOut;
    document.getElementById("req-invoice").value = req.invoice;
    document.getElementById("req-client-name").value = req.clientName;
    document.getElementById("req-hotel-name").value = req.hotelName;
    document.getElementById("req-location").value = req.location;
    document.getElementById("req-net").value = req.net;
    ...
    document.getElementById("req-discount-notice").value = req.discountNotice;
  ...
  // Builds the file directly from the full `salesData` array in memory —
  // every record, no DOM/canvas involved, so nothing gets cut off no matter
  // how many rows there are. See flights.js for the fuller explanation of
  // why this replaced the old screenshot-based PDF export.
  const CSV_COLUMNS = [
    ["id", "ID"], ["checkIn", "Check In"], ["checkOut", "Check Out"], ["invoice", "Invoice"],
    ["clientName", "Name Client"], ["hotelName", "Name Hotel"], ["location", "Location"],
    ["net", "Net"], ["netRate", "Net Rate"], ["supplier", "Supplier"], ["sellingRate", "Selling Rate"],
    ["currency", "Currency"], ["profit", "Profit"], ["customer", "Customer"], ["fileNumber", "File #"],
    ["discountNotice", "Discount Notice"], ["note", "Note"],
  ];
  ...
  const buildSalesCsv = () => {
    const rows = [...salesData].sort((a, b) => parseLocalDate(a.checkIn) - parseLocalDate(b.checkIn));
    const lines = [CSV_COLUMNS.map(([, label]) => csvEscape(label)).join(",")];

    let currentMonthYear = "";
    rows.forEach((item) => {
      const monthYear = item.checkIn
        ? parseLocalDate(item.checkIn).toLocaleString("default", { month: "long", year: "numeric" })
        : "Undated";
      if (monthYear !== currentMonthYear) {
        currentMonthYear = monthYear;
  ...
  const exportSalesDataToCsv = (filename) => {
    const csv = "\uFEFF" + buildSalesCsv(); // BOM so Excel opens UTF-8 correctly
  ...
  document.getElementById("btn-export-backup-pdf")?.addEventListener("click", () => {
    ...
    exportSalesDataToCsv(`Voyvista-Hotel-Sales-Backup-${getTodayISO()}.csv`);
  });
  ...
  document.getElementById("btn-close-annual-pdf")?.addEventListener("click", () => {
    ...
    // Data-protection policy: export only — never clears/archives data.
    // Wiping the ledger requires separate IT Admin authorization.
    exportSalesDataToCsv(`Voyvista-Hotel-Annual-Sales-Report-${getTodayISO()}.csv`);
  });
  ...
  let driveItems = JSON.parse(localStorage.getItem("vv_hotel_drive_items"));
  if (!driveItems) {
    const legacy = JSON.parse(localStorage.getItem("vv_hotel_drive_files")) || [];
    ...
    localStorage.setItem("vv_hotel_drive_items", JSON.stringify(driveItems));
  }

  let currentFolderId = null; // null = Root

  const saveDriveItems = () => localStorage.setItem("vv_hotel_drive_items", JSON.stringify(driveItems));
  ...
  document.getElementById("btn-new-folder")?.addEventListener("click", () => {
    const name = prompt("Folder name (e.g. Khalil Family, FAM-H1024):");
  ...
  function openClientDataPicker() {
    if (!window.ClientDataPicker) {
      console.error("ClientDataPicker not loaded — check that client_data_picker.js is present at /client/modules/master_data/client_data_picker.js and loaded before hotels.js.");
